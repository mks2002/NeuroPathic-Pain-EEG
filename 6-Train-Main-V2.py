...
OUTPUT_ROOT = "ML-MODEL-RESULTS-PerChannel_V3"
TEST_SIZE = 0.2
LABEL_COL = 'label'
...
def save_channel_log(channel_outdir, df, train_subjs, test_subjs):
    ...
    for s in sorted(df["subj_id"].unique(), key=int):
        ...
        lines.append(f"  subj {s}: rows 0-based [{start0},{end0}] 1-based [{start0+1},{end0+1}] total={len(idxs)}")
...
def run_models_for_channel(csv_path):
    ...
    # ------------------------------
    # CREATE CHANNEL FOLDER + SUBFOLDERS
    # ------------------------------
    ...
    train_subs, test_subs = train_test_split(subjs, test_size=TEST_SIZE, random_state=RANDOM_STATE, stratify=subj_labels)
