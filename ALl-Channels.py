# this is the main file in which all channels are processed and features are extracted 
...
MAIN_OUT_DIR = Path("ML-Features_V1")
...
def compute_features_window(x, sf, channel):
    ...
    pnorm = Pxx / Psum
    feats[f"spectral_entropy_{channel}"] = float(-np.sum(pnorm * np.log(pnorm + 1e-12)))
    cumsum = np.cumsum(Pxx)
    idx90 = np.where(cumsum >= 0.9 * np.sum(Pxx))[0]
    feats[f"spectral_edge_90_{channel}"] = float(f[idx90[0]]) if len(idx90) else float(f[-1])

    # ---------- Wavelet ----------
    try:
        coeffs = pywt.wavedec(x, 'db4', level=4)
        for i, c in enumerate(coeffs):
            feats[f"wavelet_energy_L{i}_{channel}"] = float(np.sum(np.square(c)))
    except Exception:
        for i in range(5):
    ...
    feats = {k: float(np.nan_to_num(v, nan=0.0, posinf=0.0, neginf=0.0)) for k, v in feats.items()}
    return feats
